...
class StoppingFlags(Enum):
    """ Predefined stopping flags to use when checking which streamlines should
     stop """
...
def is_outside_mask(streamlines: np.ndarray, mask: np.ndarray,
    ...
    outside : np.ndarray with shape (n_streamlines,)
        Bool array telling whether a streamline's last coordinate is outside the
        mask or not.
    """
...
def is_too_long(streamlines: np.ndarray, max_nb_steps: int = None,
    ...
    if max_length:
        output = np.logical_or(output,
                               np.array(list(length_generator(streamlines)))                                        # toDo pourquoi ça me dit une erreur ici?
                               > max_length)
...
def filter_stopping_streamlines(streamlines: np.ndarray,
