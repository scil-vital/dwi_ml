...
class AbstractDirectionGetterModel(torch.nn.Module):
    ...
    def compute_loss(self, outputs: List[Tensor],
        ...
        eos_loss: Tensor
            The EOS part of the loss at each point, stacked. (If compress_loss,
        ...
        loss, n, eos_loss = self._compute_loss(
...
class AbstractRegressionDG(AbstractDirectionGetterModel):
    ...
    def _compute_loss(self, learned_directions: Tensor, target_dirs: Tensor,
        ...
        if self.add_eos:
            ...
            return loss_dirs + self.eos_weight * loss_eos, n, loss_eos
        else:
...
class SphereClassificationDG(AbstractSphereClassificationDG):
    ...
    def _compute_loss(self, logits_per_class: Tensor, targets_idx: Tensor,
        ...
        eos_loss = None
        if self.add_eos:
            eos_loss = -learned_distribution.log_prob(
                torch.ones_like(targets_idx) * self.eos_class_idx)

        if average_results:
            return *_mean_and_weight(nll_losses), eos_loss
        else:
            n = 1
            return nll_losses, n, eos_loss
    ...
    def _compute_loss(self, learned_gaussian_params: Tuple[Tensor, Tensor],
        ...
        if self.add_eos:
            ...
            loss_eos = binary_cross_entropy_eos(learned_eos,
                                                target_dirs[:, -1],
                                                average_results)
            return nll_loss + self.eos_weight * loss_eos, n, loss_eos
        else:
