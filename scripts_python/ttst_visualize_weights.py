...
from dwi_ml.testing.visu.projects.transformer_visualisation import \
...
def main(argv):
    ...
    if args.run_locally:
        ...
        ttst_visualize_weights(args, parser)
    else:
        # 1) Finding the jupyter notebook
        dwi_ml_dir = dirname(dirname(__file__))
        ipynb_filename = join(
            dwi_ml_dir, 'dwi_ml/visu/projects/ttst_visualize_weights.ipynb')
        ...
        home = get_home()
        config_filename = join(home, 'ipynb_ttst_visualize_weights.config')
