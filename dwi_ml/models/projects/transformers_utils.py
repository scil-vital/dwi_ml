...
def add_abstract_model_args(p, uses_targets=True):
    ...
    gtt = p.add_argument_group(title='Transformer')
    gtt.add_argument(
...
def add_tto_model_args(gx, gt, gtt):
    gtt.add_argument(
...
def add_ttst_model_args(gx, gt, gtt):
    gx.add_argument(
        '--embedding_size_x', type=int, metavar='n', default=400,
        help="Embedding size for x. Will determine d_model. [%(default)s]")
    gt.add_argument(
        '--embedding_size_t', type=int, metavar='n', default=100,
        help="Embedding size for t. Default: 100. Total d_model will be "
